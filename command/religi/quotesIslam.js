...
export default {
    ...
    command: ["qislam", "quotesislam", "quoteislam"],
    description: "send a Quotes From Islam",
    name: "qislam",
    tags: "religi",

    run: async (m, { conn, usedPrefix, __dirname, text, command }) => {
        let islamQuote = [
        ...
        islamQuote = islamQuote[Math.floor(Math.random() * islamQuote.length)];
        ...
        let islam = `${islamQuote}`
        let nth = `☰ ━━┉┉┈┈ *ISLAMIC QUOTES* ┈┈┉┉━━ ☰`
        m.reply(nth + `\n` + islam)
    }
}
